...
- [x] AC-1: GIVEN `type="single"` + `collapsible` WHEN clicking a trigger THEN only that panel opens, others close
- [x] AC-2: GIVEN `type="multiple"` WHEN clicking triggers THEN multiple panels can be open simultaneously
- [x] AC-3: GIVEN an item WHEN rendered THEN it has a card background (`--background-neutral-faint-default`), rounded corners, and gap between items
- [x] AC-4: GIVEN a trigger WHEN closed THEN a `Plus` icon is shown on the right
- [x] AC-5: GIVEN a trigger WHEN expanded THEN the icon rotates 45° to become `×`
- [x] AC-6: GIVEN content WHEN expanding/collapsing THEN height animates smoothly via `--radix-accordion-content-height` CSS variable
- [x] AC-7: GIVEN keyboard focus on trigger WHEN pressing Arrow Down THEN focus moves to next trigger (Arrow Up, Home, End also work)
- [x] AC-8: GIVEN `disabled` on an item WHEN clicking THEN no expansion occurs
- [x] AC-9: GIVEN light and dark mode THEN all tokens remap correctly
- [x] AC-10: GIVEN the component THEN all values use tokens — zero hardcoded colors/sizes
- [x] AC-11: GIVEN a trigger WHEN rendered THEN min-height is `var(--layout-size-lg)` (44px touch target)

### Error Criteria (BLOCKING)

- [x] AC-E1: GIVEN `prefers-reduced-motion: reduce` WHEN expanding THEN animation slows to 400ms (consistent with project pattern — slow down, don't remove)

### Should Have

- [x] AC-12: GIVEN `defaultValue` WHEN first render THEN specified items are pre-expanded

## Scope

- [x] 1. Install `@radix-ui/react-accordion` → AC-1, AC-2
- [x] 2. Implement Accordion + AccordionItem + AccordionTrigger + AccordionContent → AC-1 through AC-8
- [x] 3. Style card items (background `--background-neutral-faint-default`, radius, gap) → AC-3
- [x] 4. Implement Plus icon rotate 45° animation (single icon, no swap) → AC-4, AC-5, AC-6
- [x] 5. Implement disabled state → AC-8
- [x] 6. Ensure trigger min-height 44px → AC-11
- [x] 7. Handle reduced motion (slow to 400ms) → AC-E1
- [x] 8. Ensure token compliance (light/dark) → AC-9, AC-10
- [x] 9. Doc page + registry + nav entries → AC-12
...
**Estimate vs Actual:** 3h → ~2.5h (83%)
**What worked:** Spec review before dev caught all the stale chevron/variant refs — zero confusion during implementation. Single Plus icon rotate 45° = elegant, simpler than +/× swap.
**What didn't:** Initial spec had too many copy-paste artifacts from a prior design direction. The spec review process (4 perspectives) was thorough but heavy for a mini-tier component.
**Next time:** For mini specs, 2 perspectives max (Frontend + Skeptic) instead of 4.
...
| ship | 2026-04-05T00:00:00Z | ~2h | Component + doc page + registry |
| review | 2026-04-07T00:00:00Z | ~20min | 0 FAIL, 2 WARN fixed |
| done | 2026-04-08T00:00:00Z | ~2.5h total | All ACs passing, shipped |
